...
int main() {
    ...
    char* mem = mmap(NULL, 1024*sizeof(char), PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS, 0, 0);
    ...
    while(1) {
        printf("External function (FUN_ADDR: %p | VAR_ADDR: %p) called, returned: %d\n", external_func, &external_func, external_func(0));
