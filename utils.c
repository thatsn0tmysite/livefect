...
export_t* find_exports(match_t* matches, bool from_disk, export_t* exports, args_t *args) {    
            ...
            if(sections[i].sh_type==SHT_SYMTAB || sections[i].sh_type==SHT_DYNSYM) {
                ...
                for (int j=0; j<symbol_num-1; j++) {
                    ...
                    if(args->arg_func_only && symtab[j].st_info != STT_FUNC)
                        continue;
                    if(args->arg_library_filter && strcmp(match->path, args->arg_library_filter)!=0)
                        continue;
                    if(args->arg_symbol_filter && strstr(symbol_names+symtab[j].st_name, args->arg_symbol_filter) == NULL)
                        continue;
...
off_t write_payload(pid_t pid, void* payload, size_t payload_len, void* dst, args_t *args) {
