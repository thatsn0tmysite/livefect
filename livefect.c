...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, short_opts, long_opts, NULL)) != -1) {
    ...
    case 'P':
      if (strlen(optarg) < 1) {
        fprintf(stderr,
                "[E] Wrong permissions lenght. Use \"rwxp\" (e.g. r-xp).\n");
        return EXIT_FAILURE;
      }

      for (int i = 0; i < strlen(args.arg_perms); i++) {
        char c = optarg[i];
        if (c == 'r' || c == 'w' || c == 'x' || c == '-' || c == 'p' ||
            c == 's' || c == '*') {
          args.arg_perms[i] = c;
        } else {
  ...
  if (args.arg_payload_path) {
    ...
    printf("[*] Loaded payload from: %s (%lu bytes)\n", args.arg_payload_path,
           payload_len);

    // We always use the same payload so source, source size and destination
    // size never change
  ...
  if (args.arg_raddr) {
    ...
    // RAW MODE
    if (write_payload(args.arg_pid, payload, payload_len, args.arg_raddr,
                      &args) == -1) {
      ...
      if (matches) {
        ...
        while (current) {
          ...
          printf("\t[+] Match (%s) for pid %d (%s) @ <%s>:%p-%p\n",
                 current->perms, current->pid, cmdline, current->path,
          ...
          if (args.arg_disasm > 0) {
            ...
            void *code_buf =
                (void *)malloc(args.arg_disasm_bytes); // this should be read
            ...
            int nread =
                process_vm_readv(current->pid, iolocal, 1, ioremote, 1, 0);
            if (nread == -1) {
              fprintf(stderr,
                      "\t\t[E] Failed reading %d bytes @ %p from remote "
                      "process %d. (errno: %d)\n",
                      args.arg_disasm_bytes, current->start, current->pid,
                      errno);
              free(code_buf);
            ...
            ZydisFormatterInit(
                &formatter,
                ZYDIS_FORMATTER_STYLE_INTEL); // TODO: support AT&T syntax via
                                              // env variable,flag or something?
            ...
            while (ZYAN_SUCCESS(ZydisDecoderDecodeFull(
                       &decoder, code_buf + offset, length - offset,
      ...
      if (exports) {
        ...
        while (current) {
          ...
          printf("\t[+] Export for pid %d (%s) %s @ %p-%p\n", current->pid,
                 current->perms, current->name, current->addr,
  ...
  if (args.arg_inject_all) {
    fprintf(stderr, "[W] Ignoring exports, injecting into matching memory "
                    "segments only!\n");
    ...
    while (current_m) {
      ...
      if (ZYAN_FAILED(ZydisEncoderNopFill(to_write, segment_size))) {
        fprintf(stderr,
                "[E] Failed to prepare buffer (nop spray) @ %p (errno: %d).\n",
                to_write, errno);
        free(to_write);
        continue;
      }
      ...
      if (memcpy(to_write, payload, payload_len) == NULL) {
        fprintf(stderr,
                "[E] Failed to prepare buffer (payload) @ %p (errno: %d).\n",
                to_write, errno);
        free(to_write);
        continue;
      }
      ...
      if (memcpy(to_write - encoded_length, encoded_instruction,
                 encoded_length) == NULL) {
        fprintf(stderr,
                "[E] Failed to prepare buffer (jmp) @ %p (errno: %d).\n",
                to_write, errno);
        free(to_write);
        continue;
      }
      ...
      if (args.arg_verbosity > 0)
        printf("[.] Attempting payload write of %s (%lu bytes) to PID: %d from "
               "%p to %p...\n",
               args.arg_payload_path, segment_size, current_m->pid,
               current_m->start, current_m->start + segment_size);

      if (write_payload(current_m->pid, to_write, segment_size,
                        current_m->start, &args) == -1) {
      ...
      // free(to_write); //TODO: why is this an invalid malloc'ed ptr?? free():
      // invalid pointer
    ...
    while (current_e) {
      if (args.arg_verbosity > 0)
        printf("[.] Attempting write of %s (%lu bytes) to PID: %d -> '%s' @ "
               "%p...\n",
               args.arg_payload_path, payload_len, current_e->pid,
               current_e->name, current_e->addr);

      int err = write_payload(current_e->pid, payload, payload_len,
                              current_e->addr, &args);
      if (err == -1) {
        fprintf(stderr, "[E] Failed to call write_payload @ %p (errno: %d).\n",
...
void print_usage() {
  ...
  printf("\t-V --process-vm:        use process_vm_writev instead of "
         "/proc/[pid]/mem\n");
  printf(
      "\t-p --pid PID:           run against specified pid (default: NULL)\n");
  printf("\t-f --file FILE:         file containing data to write/payload "
         "(default: NULL)\n");
  printf("\t-a --addr ADDR:         (raw mode) address to write data/payload "
         "to\n");
  printf("\t-v --verbosity LEVEL:   set verbosity level (default: 0)\n");
  printf("\t-P --perms PERMS:       which permissions to look for (default: "
         "rwxp)\n");
  printf("\t-S --func-only:         only display function symbols\n");
  printf(
      "\t   --symbol SYMBOL:     write FILE to all symbols with name SYMBOL\n");
  printf("\t   --library LIBRARY:   to use with --symbol allow write to "
         "specific SYMBOL from specific LIBRARY\n");
  printf("\t-D --data-only:         only display data symbols\n");
  printf("\t-I --inject-all:        (potentially unsafe) force payload "
         "injection in any matching memory area\n");

  printf("\t-m --maps:              only list memory maps\n");
  printf("\t   --disasm N:          (implies -m) try disassemble the first N "
         "instructions and show output (default: 0)\n");
  printf("\t   --disasm-bytes N:    (implies -m) bytes to pass to disassembler "
         "(default: 64)\n");
  ...
  // TODO: check if there's a process_vm_mmap or similar to change maps
  // permissions before writing and add option to do so
  // https://man7.org/linux/man-pages/man2/pidfd_open.2.html
  // https://lwn.net/Articles/830648/
  printf("\nMade while screaming in *PANIC* by @thatsn0mysite "
         "(https://thatsn0tmy.site)\n");
}
